...
void compnorm(const FloatType* ver1, const FloatType* ver2, 
	const FloatType* ver3, FloatType* location, bool clockwise = false)
{
	...
	FloatType norm = normal[1] * normal[1] 
		+ normal[2] * normal[2] 
		+ normal[0] * normal[0];
	if (norm != FloatType(0))
	{
		FloatType temp = FloatType(1) / sqrt(norm);
		normal[0] *= temp;
		normal[1] *= temp;
		normal[2] *= temp;
	}
...
struct MeshData
{
	...
	{
		face_normals.clear();
		face_normals.reserve(n_faces());
		for (size_t i = 0; i < n_faces(); i++)
		{
			FaceT &face = faces[i];
			unsigned int v_idx0 = face[0];
			unsigned int v_idx1 = face[1];
			unsigned int v_idx2 = face[2];
			NormalT normal(3);
			compnorm(&vertices[v_idx0][0], &vertices[v_idx1][0],
				&vertices[v_idx2][0], &normal[0]);
	...
	void compute_vertex_normals()
	{
		...
		vertex_normals.resize(n_vertices(), NormalT(3));
		vector<unsigned int> n_neighbour_faces;
		n_neighbour_faces.resize(n_vertices(), 0);
		for (size_t i = 0; i < n_faces(); i++)
		{
			NormalT &f_normal = face_normals[i];

			FaceT &face = faces[i];
			for (size_t j = 0; j < face.size(); j++)
			{
				unsigned int v_idx = face[j];

				vertex_normals[v_idx][0] += f_normal[0];
				vertex_normals[v_idx][1] += f_normal[1];
				vertex_normals[v_idx][2] += f_normal[2];

				n_neighbour_faces[v_idx]++;
			}
		}

		for (size_t i = 0; i < n_vertices(); i++)
		{
			vertex_normals[i][0] /= n_neighbour_faces[i];
			vertex_normals[i][1] /= n_neighbour_faces[i];
			vertex_normals[i][2] /= n_neighbour_faces[i];
		}
	}
	...
	void readPLY(const std::string& filename)
	{
		...
		ply = ply_open_for_reading(filename.c_str(), &nelems, &elist, 
			&file_type, &version);
		...
		for (int i = 0; i < nelems; i++) {
			...
			if (equal_strings(ply::elem_names[0], elem_name)) {
				...
				{
				case PLY_VERTEX_RGB:
					ply_read_vertices<ply::VertexColor>(
						ply, vertex_type, num_vertices, *this);
					break;
				case PLY_VERTEX_NORMAL_RGB:
					ply_read_vertices<ply::VertexNormalColor>(
						ply, vertex_type, num_vertices, *this);
					break;
				case PLY_VERTEX_RGBA:
					ply_read_vertices<ply::VertexColorAlpha>(
						ply, vertex_type, num_vertices, *this);
					break;
				case PLY_VERTEX_NORMAL_RGBA:
					ply_read_vertices<ply::VertexNormalColorAlpha>(
						ply, vertex_type, num_vertices, *this);
					break;
				case PLY_VERTEX_RGB_SPECULAR:
					ply_read_vertices<ply::VertexColorSpecular>(
						ply, vertex_type, num_vertices, *this);
					break;
				case PLY_VERTEX_NORMAL_RGB_SPECULAR:
					ply_read_vertices<ply::VertexNormalColorSpecular>(
						ply, vertex_type, num_vertices, *this);
					break;
				case PLY_VERTEX_RGBA_SPECULAR:
					ply_read_vertices<ply::VertexColorAlphaSpecular>(
						ply, vertex_type, num_vertices, *this);
					break;
				case PLY_VERTEX_NORMAL_RGBA_SPECULAR:
					ply_read_vertices<ply::VertexNormalColorAlphaSpecular>(
						ply, vertex_type, num_vertices, *this);
					break;
				default:
					break;
		...
		bool save_specular, bool save_mesh_binary)
	{
		...
		vector<FaceT>::const_iterator f_it;
		vector<FaceT>::const_iterator f_end = faces.end();
